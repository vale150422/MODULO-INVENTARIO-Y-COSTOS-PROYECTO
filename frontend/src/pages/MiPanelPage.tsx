...
export default function MiPanelPage() {
  ...
  const cambiarPassword = async (e: React.FormEvent) => {
    ...
    if (passNueva !== passConfirm) {
      setMsgTipo('error'); setMsgPass('Las contraseñas no coinciden'); return;
    }
    if (passNueva.length < 6) {
      setMsgTipo('error'); setMsgPass('La contraseña debe tener al menos 6 caracteres'); return;
    }
    setLoading(true);
    try {
      ...
      setMsgTipo('error');
      setMsgPass(err.message === 'Failed to fetch'
      ...
      <div className="grid grid-cols-2 gap-6">
          ...
          <div className="space-y-3">
            {[
              ...
              <div key={item.label}
                ...
                <span className="text-xs text-[#6b8c3e] uppercase tracking-widest font-semibold">
                  {item.label}
                </span>
        ...
        <div className="bg-white border border-[#e0ead0] rounded-xl p-5 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            ...
            <button
              onClick={() => { setShowCambiarPass(!showCambiarPass); setMsgPass(''); }}
              style={{
                background: showCambiarPass ? '#6b7280' : '#2d4a1e',
                color: '#ffffff',
                border: 'none',
                borderRadius: '8px',
                padding: '6px 14px',
